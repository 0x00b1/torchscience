...
def elliptic_minimum_order(
    ...
    """
    Compute minimum elliptic filter order for given specifications.

    Returns the lowest order digital elliptic filter that loses no more
    ...
    natural_frequency : float
        Natural frequency for the filter, which equals the passband edge
        frequency. For bandpass/bandstop, this is a list [low, high].

    Notes
    -----
    Elliptic filters have equiripple in both passband and stopband, providing
    the steepest rolloff for a given filter order. This function computes
    the minimum order needed to meet the specifications.

    The order is computed using the degree equation involving complete
    ...
    if filter_type == "lowpass":
        freq_ratio = wp_warped / ws_warped  # k = wp/ws < 1
        ...
        freq_ratio = 1.0 / min(s_low, s_high)
        ...
        freq_ratio = min(p_low, p_high)
    else:
        raise ValueError(f"Invalid filter_type: {filter_type}")

    # k is the selectivity (wp/ws for lowpass)
    k = freq_ratio
    kp = math.sqrt(1 - k**2)  # complementary

    # Compute ripple parameters
    eps_p = math.sqrt(10 ** (passband_ripple_db / 10) - 1)
    eps_s = math.sqrt(10 ** (stopband_attenuation_db / 10) - 1)

    # k1 is the ripple ratio
    k1 = eps_p / eps_s
    k1p = math.sqrt(1 - k1**2)

    # Order formula using elliptic integrals:
    # n = K(k) * K'(k1) / (K'(k) * K(k1))
    K_k = float(special.ellipk(k**2))
    Kp_k = float(special.ellipk(kp**2))
    K_k1 = float(special.ellipk(k1**2))
    Kp_k1 = float(special.ellipk(k1p**2))

    order = (K_k * Kp_k1) / (Kp_k * K_k1)
    order = int(math.ceil(abs(order)))

    # Natural frequency is the passband edge
    wn_digital = passband_frequency
