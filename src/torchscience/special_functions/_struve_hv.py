...
def struve_hv(nu: Tensor, z: Tensor, *, tol: float = 1e-12, max_terms: int = 4096) -> Tensor:
    r"""
    Struve function H_ν(z) via 1F2 representation.
    ...
    This implementation evaluates 1F2 using the generalized series evaluator with
    adaptive truncation and forms the prefactor in log-space using ``gammaln``.
    ...
    # Prefactor: 2 * (z/2)^{nu+1} / (sqrt(pi) * Gamma(nu+3/2)) computed via gammaln
