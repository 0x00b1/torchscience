"""Inverse Discrete Sine Transform (IDST) implementation with new contract."""
...
def inverse_sine_transform(
    ...
    *,
    ...
    This is the inverse of the DST. The inverse relationships are:

    - IDST-I is DST-I (DST-I is its own inverse, up to scaling)
    - IDST-II is DST-III
    - IDST-III is DST-II
    - IDST-IV is DST-IV (DST-IV is its own inverse)
    ...
    type : int, optional
        IDST type (1, 2, 3, or 4). IDST-II is computed via DST-III,
        and IDST-III via DST-II. DST-I and DST-IV are self-inverse.
        Default: ``2``.
    dim : int or tuple of int, optional
        The dimension(s) along which to compute the transform.
        If a tuple, computes a multi-dimensional IDST by applying
        the 1D IDST sequentially along each dimension.
    ...
    -------
    Tensor
        The IDST of the input. Same dtype as input (real-valued).
    ...
    >>> x_rec = inverse_sine_transform(X, norm='ortho')
    ...
    See Also
    --------
    sine_transform : The forward DST.
    scipy.fft.idst : SciPy's IDST implementation.
    """
    # Validate input is real
    if input.is_complex():
        raise ValueError(
            "inverse_sine_transform requires real-valued input, "
    ...
    if len(dim_tuple) == 1:
        # 1D IDST
        result = torch.ops.torchscience.inverse_fourier_sine_transform(
            x,
        ...
        # Multi-dimensional IDST: apply 1D IDST sequentially along each dimension
        result = x
        for d in dim_tuple:
            result = torch.ops.torchscience.inverse_fourier_sine_transform(
                result,
    ...
    # Apply window if provided (after IDST for inverse transform)
...
inverse_fourier_sine_transform = inverse_sine_transform
