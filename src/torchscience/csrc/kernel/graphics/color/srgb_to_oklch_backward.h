...
#include "srgb_to_oklab_backward.h"

namespace torchscience::kernel::graphics::color {

/**
 * Backward pass for sRGB to Oklch conversion.
 *
 * Computes gradients w.r.t. sRGB input given gradients w.r.t. Oklch output.
 * Uses chain rule through the composition: sRGB -> Oklab -> Oklch
 *
 * Derivatives for Oklab -> Oklch:
 * - dL/dL_lab = 1
 * - dC/da = a/C, dC/db = b/C (handle C=0 case)
 * - dh/da = -b/(a^2 + b^2), dh/db = a/(a^2 + b^2) (handle a=b=0 case)
 *
 * @param grad_lch Gradient w.r.t. Oklch output [dL, dC, dh]
 * @param rgb Original sRGB input [R, G, B]
 * @param grad_rgb Output gradient w.r.t. sRGB [dR, dG, dB]
 */
template <typename T>
void srgb_to_oklch_backward_scalar(const T* grad_lch, const T* rgb, T* grad_rgb) {
    // Forward pass: compute Oklab from sRGB
    T lab[3];
