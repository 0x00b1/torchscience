...
/**
 * Compute backward pass for dilation.
 *
 * Gradient flows through the argmax indicator:
 *   grad_input[argmax_position] += grad_output[output_position]
 *
 ...
 * @param input_ptr Input tensor data (needed to find argmax)
...
void dilation_backward_scalar(
    ...
    int64_t argmax_offset = -1;

    // Iterate over all positions in the structuring element to find argmax
    ...
    for (int64_t se_idx = 0; se_idx < total_se_elements; ++se_idx) {
        ...
        if (val > max_val) {
            max_val = val;
            argmax_offset = from_padding ? -1 : current_offset;
        }
    }

    // Accumulate gradient at argmax position
    if (argmax_offset >= 0) {
        grad_input_ptr[argmax_offset] += grad_output_val;
    }
}
