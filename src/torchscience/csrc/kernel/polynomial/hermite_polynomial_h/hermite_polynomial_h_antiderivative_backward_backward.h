...
// Second-order backward for Hermite H polynomial antiderivative
//
...
template <typename T>
void hermite_polynomial_h_antiderivative_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    const T* grad_output,
    const T* coeffs,
    ...
    // grad_grad_output[k+1] = gg_coeffs[k] / (2*(k+1))
    for (int64_t k = 0; k < N; ++k) {
        if (k + 1 < output_size) {
            grad_grad_output[k + 1] = gg_coeffs[k] / T(2 * (k + 1));
        }
    }
}
...
void hermite_polynomial_h_antiderivative_backward_backward(
    ...
    for (int64_t k = 0; k < N; ++k) {
        if (k + 1 < output_size) {
            grad_grad_output[k + 1] = gg_coeffs[k] / c10::complex<T>(T(2 * (k + 1)), T(0));
        }
    }
}
