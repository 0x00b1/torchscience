...
// Backward pass for Chebyshev U polynomial multiply-by-x.
//
// Forward operation:
//   output[1] += 0.5 * coeffs[0]                     (from U_0 term)
//   output[k+1] += 0.5 * coeffs[k]  for k >= 1       (from U_k terms)
//   output[k-1] += 0.5 * coeffs[k]  for k >= 1       (from U_k terms)
//
// Backward pass reverses this:
//   grad_coeffs[0] = 0.5 * grad_output[1]
...
template <typename T>
void chebyshev_polynomial_u_mulx_backward(
    T* grad_coeffs,
    const T* grad_output,
    const T* coeffs,
    int64_t N,
    ...
    // Backward for: output[1] += 0.5 * coeffs[0]
    if (1 < output_size) {
