...
//   N, M, K: sizes
template <typename T>
void polynomial_subtract_backward_backward(
    T* grad_grad_output,
    T* grad_p,
    T* grad_q,
    const T* gg_p,
    ...
    // grad_grad_output accumulates contributions from gg_p and -gg_q
    // Since grad_p = grad_output[:N] and grad_q = -grad_output[:M]:
    //   d(grad_p)/d(grad_output) = 1 for k < N
    //   d(grad_q)/d(grad_output) = -1 for k < M
    // So: grad_grad_output += gg_p (for k < N)
    //     grad_grad_output -= gg_q (for k < M)  -- because d(grad_q)/d(grad_output) = -1
    for (int64_t k = 0; k < K; ++k) {
        T val = T(0);
        if (k < N) val += gg_p[k];
        if (k < M) val -= gg_q[k];  // Subtract because grad_q = -grad_output[:M]
