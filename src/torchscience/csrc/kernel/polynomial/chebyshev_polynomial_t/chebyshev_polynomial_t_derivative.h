...
// Chebyshev T polynomial derivative using recurrence relation:
// For Chebyshev T polynomial with coefficients c[0..N-1], the derivative
// has coefficients d[0..N-2] computed via backward recurrence:
//
//   d_{n-1} = 2*n*c_n
//   d_k = d_{k+2} + 2*(k+1)*c_{k+1}  for k = n-2, ..., 1
//   d_0 = 0.5*d_2 + c_1
//
// This computes the Chebyshev coefficients of the derivative polynomial.
//
// Parameters:
//   output: array of size max(N-1, 1)
//   coeffs: input Chebyshev coefficients, size N
//   N: number of input coefficients
//
// Returns: size of output (max(N-1, 1))
template <typename T>
int64_t chebyshev_polynomial_t_derivative(T* output, const T* coeffs, int64_t N) {
    if (N <= 1) {
        output[0] = T(0);
        return 1;
    }
...
template <typename T>
int64_t chebyshev_polynomial_t_derivative(
