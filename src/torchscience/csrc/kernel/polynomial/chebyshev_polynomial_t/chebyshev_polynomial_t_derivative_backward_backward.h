...
// Second-order backward for Chebyshev T polynomial derivative.
//
...
template <typename T>
void chebyshev_polynomial_t_derivative_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    int64_t N,
    int64_t output_size
    ...
    // d_k = d_{k+2} + 2*(k+1)*gg_coeffs[k+1] for k = deg-2 down to 1
    ...
    // d_0 = gg_coeffs[1] + 0.5*d_2
    if (output_size >= 1) {
        grad_grad_output[0] = T(0);
        if (N >= 2) {
            grad_grad_output[0] = gg_coeffs[1];
        }
        if (output_size >= 3) {
            grad_grad_output[0] = grad_grad_output[0] + T(0.5) * grad_grad_output[2];
        }
    }
}
...
void chebyshev_polynomial_t_derivative_backward_backward(
    ...
    if (output_size >= 1) {
        grad_grad_output[0] = zero;
        if (N >= 2) {
            grad_grad_output[0] = gg_coeffs[1];
        }
        if (output_size >= 3) {
            grad_grad_output[0] = grad_grad_output[0] + half * grad_grad_output[2];
        }
    }
}
