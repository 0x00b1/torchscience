...
// Backward pass for Chebyshev T polynomial multiply-by-x.
//
// Forward operation:
//   output[1] += coeffs[0]                           (from T_0 term)
//   output[k+1] += 0.5 * coeffs[k]  for k >= 1       (from T_k terms)
//   output[k-1] += 0.5 * coeffs[k]  for k >= 1       (from T_k terms)
//
...
template <typename T>
void chebyshev_polynomial_t_mulx_backward(
    T* grad_coeffs,
    const T* grad_output,
    const T* coeffs,
    int64_t N,
    ...
    // Backward for: output[1] += coeffs[0]
    if (1 < output_size) {
        grad_coeffs[0] += grad_output[1];
    }
