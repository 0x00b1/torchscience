...
// Backward pass for Chebyshev T polynomial antiderivative.
//
...
//   da_0/da_k = -sign_k for k even, k >= 2
//   So da_0/dc_i = sum_{k even, k>=2} (-sign_k) * (da_k/dc_i)
//
// Parameters:
...
template <typename T>
void chebyshev_polynomial_t_antiderivative_backward(
    T* grad_coeffs,
    const T* grad_output,
    const T* coeffs,
    int64_t N
    ...
    // a_0 = -sum_{k=2,4,6,...} sign_k * a_k where sign_k = (-1)^{k/2}
    ...
    // Also, if N is even, a_N affects a_0:
    // a_0 depends on a_N with coefficient -sign_N where sign_N = (-1)^{N/2}
    if (N % 2 == 0) {
        int64_t sign_N = ((N / 2) % 2 == 0) ? 1 : -1;
        // da_0/da_N = -sign_N, so da_0/dc_{N-1} = -sign_N / (2*N)
    ...
    for (int64_t k = 2; k < N; ++k) {
        ...
        // Indirect contribution through a_0 if k is even
        if (k % 2 == 0) {
            int64_t sign_k = ((k / 2) % 2 == 0) ? 1 : -1;
            // da_0/da_k = -sign_k
    ...
    // a_0 also has contribution from a_0 (k=0), but T_0(0) = 1 and sign = 1
    // This is just the direct term, no indirect chain through other a_k
}
