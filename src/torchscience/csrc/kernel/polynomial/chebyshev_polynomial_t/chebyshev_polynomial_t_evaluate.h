...
// Clenshaw's algorithm for Chebyshev T polynomial evaluation
// Evaluates p(x) = sum_{k=0}^{N-1} c_k * T_k(x)
...
// Returns: p(x)
template <typename T>
T chebyshev_polynomial_t_evaluate(const T* coeffs, T x, int64_t N) {
    if (N == 0) {
        return T(0);
    }
    if (N == 1) {
...
template <typename T>
c10::complex<T> chebyshev_polynomial_t_evaluate(
