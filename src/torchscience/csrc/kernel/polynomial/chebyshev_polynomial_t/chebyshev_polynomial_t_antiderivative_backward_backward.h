...
// Second-order backward for Chebyshev T polynomial antiderivative.
//
...
template <typename T>
void chebyshev_polynomial_t_antiderivative_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    const T* grad_output,
    const T* coeffs,
    ...
    // a_0 = -sum_{k even, k>0} sign_k * a_k
    T p_at_zero = T(0);
