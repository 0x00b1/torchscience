...
// Second-order backward for Hermite He polynomial mulx
//
// Backward: grad_coeffs[0] = grad_output[1]
...
//
// d(grad_coeffs[0])/d(grad_output[1]) = 1
// d(grad_coeffs[k])/d(grad_output[k-1]) = k for k >= 1
// d(grad_coeffs[k])/d(grad_output[k+1]) = 1 for k >= 1
//
// grad_grad_output[j] = gg_coeffs[j-1] (from k=j-1, if j >= 1 and k < N)
...
template <typename T>
void hermite_polynomial_he_mulx_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    const T* grad_output,
    const T* coeffs,
    ...
    // From grad_coeffs[0] = grad_output[1]:
    // grad_grad_output[1] += gg_coeffs[0]
    grad_grad_output[1] = grad_grad_output[1] + gg_coeffs[0];

    // From grad_coeffs[k] = k * grad_output[k-1] + grad_output[k+1] for k >= 1:
    for (int64_t k = 1; k < N; ++k) {
        // grad_grad_output[k-1] += k * gg_coeffs[k]
        grad_grad_output[k - 1] = grad_grad_output[k - 1] + T(k) * gg_coeffs[k];
        // grad_grad_output[k+1] += gg_coeffs[k]
        if (k + 1 < output_size) {
            grad_grad_output[k + 1] = grad_grad_output[k + 1] + gg_coeffs[k];
        }
    }
}
