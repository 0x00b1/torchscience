...
// Second-order backward for Chebyshev V polynomial derivative.
//
...
template <typename T>
void chebyshev_polynomial_v_derivative_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    int64_t N,
    int64_t output_size
    ...
    // d_0 = 2*gg_coeffs[1] + d_2
    if (output_size >= 1) {
        grad_grad_output[0] = T(0);
        if (N >= 2) {
            grad_grad_output[0] = T(2) * gg_coeffs[1];
        }
        if (output_size >= 3) {
            grad_grad_output[0] = grad_grad_output[0] + grad_grad_output[2];
        }
    }
}
...
void chebyshev_polynomial_v_derivative_backward_backward(
    ...
    if (output_size >= 1) {
        ...
        if (N >= 2) {
            grad_grad_output[0] = c10::complex<T>(T(2), T(0)) * gg_coeffs[1];
        }
        if (output_size >= 3) {
            grad_grad_output[0] = grad_grad_output[0] + grad_grad_output[2];
        }
