...
// Backward pass for Chebyshev V polynomial multiply-by-x.
//
// Forward operation:
//   output[0] += 0.5 * coeffs[0]                   (from V_0 term)
//   output[1] += 0.5 * coeffs[0]                   (from V_0 term)
//   output[k+1] += 0.5 * coeffs[k]  for k >= 1     (from V_k terms)
//   output[k-1] += 0.5 * coeffs[k]  for k >= 1     (from V_k terms)
//
// Backward pass reverses this:
//   grad_coeffs[0] = 0.5 * (grad_output[0] + grad_output[1])
...
template <typename T>
void chebyshev_polynomial_v_mulx_backward(
    T* grad_coeffs,
    const T* grad_output,
    const T* coeffs,
    int64_t N,
