...
// Second-order backward for Chebyshev V polynomial multiply-by-x.
//
...
template <typename T>
void chebyshev_polynomial_v_mulx_backward_backward(
    T* grad_grad_output,
    ...
    // output[0] += 0.5 * coeffs[0]
    // output[1] += 0.5 * coeffs[0]
    // output[k+1] += 0.5 * coeffs[k]  for k >= 1
    // output[k-1] += 0.5 * coeffs[k]  for k >= 1

    // From gg_coeffs[0]: contributes to grad_grad_output[0] and grad_grad_output[1]
    if (0 < output_size) {
        grad_grad_output[0] += T(0.5) * gg_coeffs[0];
    }
    if (1 < output_size) {
        grad_grad_output[1] += T(0.5) * gg_coeffs[0];
    }
...
void chebyshev_polynomial_v_mulx_backward_backward(
    ...
    // From gg_coeffs[0]: contributes to grad_grad_output[0] and grad_grad_output[1]
    if (0 < output_size) {
        grad_grad_output[0] += half * gg_coeffs[0];
    }
    if (1 < output_size) {
        grad_grad_output[1] += half * gg_coeffs[0];
    }
