...
// Clenshaw's algorithm for Chebyshev W polynomial evaluation
// Evaluates p(x) = sum_{k=0}^{N-1} c_k * W_k(x)
//
// W polynomials: W_0(x) = 1, W_1(x) = 2x + 1, W_{n+1}(x) = 2x*W_n(x) - W_{n-1}(x)
//
...
// Returns: p(x)
template <typename T>
T chebyshev_polynomial_w_evaluate(const T* coeffs, T x, int64_t N) {
    if (N == 0) {
        return T(0);
    }
    if (N == 1) {
...
template <typename T>
c10::complex<T> chebyshev_polynomial_w_evaluate(
