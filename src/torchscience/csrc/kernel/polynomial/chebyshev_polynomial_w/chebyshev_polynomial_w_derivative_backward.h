...
// Backward pass for Chebyshev W polynomial derivative.
//
// Forward recurrence (from chebyshev_polynomial_w_derivative):
...
template <typename T>
void chebyshev_polynomial_w_derivative_backward(
    T* grad_coeffs,
    const T* grad_output,
    int64_t N,
    int64_t output_size
    ...
    // We'll process forward through k from 0 to deg-1, accumulating into grad_d[k+2]
    // Actually, process backward: grad_d[k] flows to grad_d[k+2] in the forward,
    // so in backward, grad_d[k+2] receives contribution from grad_d[k]

    // Simple approach: accumulate gradients in forward order
    // grad_d starts as grad_output, then we propagate
    ...
    if (output_size >= 1) {
        if (N >= 2) {
            grad_coeffs[1] += grad_d[0];
        }
        if (output_size >= 3) {
            grad_d[2] += T(0.5) * grad_d[0];
        }
    }
