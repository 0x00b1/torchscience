...
// Second-order backward for Chebyshev W polynomial antiderivative.
//
...
template <typename T>
void chebyshev_polynomial_w_antiderivative_backward_backward(
    T* grad_grad_output,
    const T* gg_coeffs,
    const T* grad_output,
    const T* coeffs,
    ...
    if (N >= 1) {
        ...
        grad_grad_output[1] = gg_coeffs[0] - T(0.5) * c_2;
    }

    // a_0 = -sum_k sign_k * a_k
    T p_at_zero = T(0);
    for (int64_t k = 0; k <= N; k++) {
        int64_t sign = get_sign(k);
...
void chebyshev_polynomial_w_antiderivative_backward_backward(
    ...
    if (N >= 1) {
        ...
        grad_grad_output[1] = gg_coeffs[0] - half * c_2;
    }
