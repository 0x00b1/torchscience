// src/torchscience/csrc/autograd/geometry/intersection/ray_aabb.h
#pragma once
...
/**
 * Autograd function for ray-AABB intersection.
 *
 * Computes the intersection of rays with axis-aligned bounding boxes.
 */
class RayAABBFunction : public torch::autograd::Function<RayAABBFunction> {
 public:
  static std::vector<at::Tensor> forward(
    ...
    ctx->save_for_backward({origins, directions, box_min, box_max});

    at::AutoDispatchBelowAutograd guard;

    auto result = c10::Dispatcher::singleton()
                      .findSchemaOrThrow("torchscience::ray_aabb", "")
                      .typed<std::tuple<
                          at::Tensor,
                          at::Tensor,
                          at::Tensor,
                          ...
                          const at::Tensor&,
                          const at::Tensor&)>()
                      .call(origins, directions, box_min, box_max);
  ...
  static std::vector<at::Tensor> backward(
    ...
    at::Tensor box_min = saved[2];
    at::Tensor box_max = saved[3];
    ...
    auto result =
        c10::Dispatcher::singleton()
            .findSchemaOrThrow("torchscience::ray_aabb_backward", "")
    ...
    return {
        ...
        std::get<2>(result), // grad_box_min
        std::get<3>(result)  // grad_box_max
    };
  }
};
...
ray_aabb(
    ...
    const at::Tensor& box_min,
    const at::Tensor& box_max) {
  auto result = RayAABBFunction::apply(
...
TORCH_LIBRARY_IMPL(torchscience, Autograd, m) {
  m.impl(
      "ray_aabb",
      &torchscience::autograd::geometry::intersection::ray_aabb);
}
