...
#include "../../kernel/information_theory/transfer_entropy.h"
#include "../../kernel/information_theory/transfer_entropy_backward.h"

namespace torchscience::cpu::information_theory {

namespace {

inline at::Tensor te_preprocess_input(
...
inline at::Tensor te_apply_reduction(
...
inline double te_get_log_base_scale(c10::optional<double> base) {
    if (!base.has_value()) {
        return 1.0;
    }
    double b = base.value();
...
inline at::Tensor transfer_entropy(
    ...
    at::Tensor joint_prob = te_preprocess_input(joint, input_type);
    double log_base_scale = te_get_log_base_scale(base);

    // The last 3 dimensions are (y_t, y_prev, x_prev)
    // All preceding dimensions are batch dimensions
    int64_t size_yt = joint_prob.size(-3);
    int64_t size_yprev = joint_prob.size(-2);
    int64_t size_xprev = joint_prob.size(-1);
    ...
    at::Tensor joint_t = joint_prob.contiguous().view({batch_size, size_yt, size_yprev, size_xprev});
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        "transfer_entropy_cpu",
        [&]() {
            const scalar_t* joint_ptr = joint_t.data_ptr<scalar_t>();
            scalar_t* out_ptr = output.data_ptr<scalar_t>();
            scalar_t scale = static_cast<scalar_t>(log_base_scale);

            int64_t joint_stride = size_yt * size_yprev * size_xprev;

            at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
                // Allocate marginals per thread
                std::vector<scalar_t> p_yprev(size_yprev);
                std::vector<scalar_t> p_yprev_xprev(size_yprev * size_xprev);
                std::vector<scalar_t> p_yt_yprev(size_yt * size_yprev);

                for (int64_t idx = begin; idx < end; ++idx) {
                    const scalar_t* batch_joint = joint_ptr + idx * joint_stride;

                    // Initialize marginals
                    std::fill(p_yprev.begin(), p_yprev.end(), scalar_t(0));
                    std::fill(p_yprev_xprev.begin(), p_yprev_xprev.end(), scalar_t(0));
                    std::fill(p_yt_yprev.begin(), p_yt_yprev.end(), scalar_t(0));

                    // Compute marginals by summing
                    // joint has shape [size_yt, size_yprev, size_xprev]
                    // p_yprev = sum over yt and xprev
                    // p_yprev_xprev = sum over yt
                    // p_yt_yprev = sum over xprev
                    for (int64_t yt = 0; yt < size_yt; ++yt) {
                        for (int64_t yp = 0; yp < size_yprev; ++yp) {
                            for (int64_t xp = 0; xp < size_xprev; ++xp) {
                                scalar_t p = batch_joint[(yt * size_yprev + yp) * size_xprev + xp];
                                p_yprev[yp] += p;
                                p_yprev_xprev[yp * size_xprev + xp] += p;
                                p_yt_yprev[yt * size_yprev + yp] += p;
                            }
                        }
                    }

                    out_ptr[idx] = torchscience::kernel::information_theory::transfer_entropy_kernel<scalar_t>(
                        batch_joint,
                        p_yprev_xprev.data(),
                        p_yt_yprev.data(),
                        p_yprev.data(),
    ...
    return te_apply_reduction(output, reduction);
}

inline at::Tensor transfer_entropy_backward(
        ...
        [&]() {
            ...
            at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
                ...
                for (int64_t idx = begin; idx < end; ++idx) {
                    ...
                    torchscience::kernel::information_theory::transfer_entropy_backward_kernel<scalar_t>(
                        grad_val,
                        batch_joint,
                        p_yprev_xprev.data(),
                        p_yt_yprev.data(),
    ...
    output_shape.push_back(size_xprev);
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
    m.impl("transfer_entropy", &torchscience::cpu::information_theory::transfer_entropy);
    m.impl("transfer_entropy_backward", &torchscience::cpu::information_theory::transfer_entropy_backward);
}
