...
inline std::tuple<at::Tensor, at::Tensor, at::Tensor> renyi_divergence_backward_backward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        p_prob.scalar_type(),
        "renyi_divergence_backward_backward_cpu",
        [&]() {
            const scalar_t* p_ptr = p_t.data_ptr<scalar_t>();
            const scalar_t* q_ptr = q_t.data_ptr<scalar_t>();
            ...
            if (reduction == "none") {
                at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
                    for (int64_t idx = begin; idx < end; ++idx) {
                        ...
                        torchscience::kernel::information_theory::renyi_divergence_backward_backward_kernel<scalar_t>(
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
    ...
    m.impl("renyi_divergence_backward_backward", &torchscience::cpu::information_theory::renyi_divergence_backward_backward);
}
