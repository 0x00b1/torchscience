...
#include "../../../kernel/polynomial/laguerre_polynomial_l/laguerre_polynomial_l_evaluate.h"
#include "../../../kernel/polynomial/laguerre_polynomial_l/laguerre_polynomial_l_evaluate_backward.h"
#include "../../../kernel/polynomial/laguerre_polynomial_l/laguerre_polynomial_l_evaluate_backward_backward.h"
        ...
        [&] {
            ...
            at::parallel_for(0, B * M, 1, [&](int64_t start, int64_t end) {
                for (int64_t idx = start; idx < end; ++idx) {
                    int64_t b = idx / M;
                    int64_t m = idx % M;
                    out_ptr[idx] = kernel::polynomial::laguerre_polynomial_l_evaluate(
                        coeffs_ptr + b * N,
                        x_ptr[m],
                        N
                    );
...
inline std::tuple<at::Tensor, at::Tensor> laguerre_polynomial_l_evaluate_backward(
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "laguerre_polynomial_l_evaluate_backward",
        [&] {
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            const scalar_t* x_ptr = x_contig.data_ptr<scalar_t>();
            ...
            for (int64_t b = 0; b < B; ++b) {
                for (int64_t m = 0; m < M; ++m) {
                    ...
                    scalar_t local_grad_x = kernel::polynomial::laguerre_polynomial_l_evaluate_backward(
...
inline std::tuple<at::Tensor, at::Tensor, at::Tensor> laguerre_polynomial_l_evaluate_backward_backward(
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "laguerre_polynomial_l_evaluate_backward_backward",
        [&] {
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            const scalar_t* x_ptr = x_contig.data_ptr<scalar_t>();
            const scalar_t* grad_out_ptr = grad_output_flat.data_ptr<scalar_t>();
            ...
            for (int64_t b = 0; b < B; ++b) {
                for (int64_t m = 0; m < M; ++m) {
                    ...
                    auto [ggo, g_x] = kernel::polynomial::laguerre_polynomial_l_evaluate_backward_backward(
                        temp_grad_coeffs.data(),
                        gg_coeffs_ptr + b * N,
                        gg_x_ptr[m],
                        grad_out_ptr[b * M + m],
...
TORCH_LIBRARY_IMPL(torchscience, CPU, module) {
    module.impl("laguerre_polynomial_l_evaluate", torchscience::cpu::polynomial::laguerre_polynomial_l_evaluate);
    module.impl("laguerre_polynomial_l_evaluate_backward", torchscience::cpu::polynomial::laguerre_polynomial_l_evaluate_backward);
    module.impl("laguerre_polynomial_l_evaluate_backward_backward", torchscience::cpu::polynomial::laguerre_polynomial_l_evaluate_backward_backward);
}
