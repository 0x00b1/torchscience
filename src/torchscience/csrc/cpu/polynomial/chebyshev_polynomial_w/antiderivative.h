#pragma once

#include <ATen/ATen.h>
#include <ATen/Dispatch.h>
#include <ATen/Parallel.h>
#include <torch/library.h>

#include "../../../kernel/polynomial/chebyshev_polynomial_w/chebyshev_polynomial_w_antiderivative.h"
#include "../../../kernel/polynomial/chebyshev_polynomial_w/chebyshev_polynomial_w_antiderivative_backward.h"
#include "../../../kernel/polynomial/chebyshev_polynomial_w/chebyshev_polynomial_w_antiderivative_backward_backward.h"

namespace torchscience::cpu::polynomial {

// Forward: coeffs (B, N) -> output (B, N + 1)
// Computes the antiderivative of a Chebyshev W polynomial
inline at::Tensor chebyshev_polynomial_w_antiderivative(
    const at::Tensor& coeffs
) {
    TORCH_CHECK(coeffs.dim() >= 1, "coeffs must have at least 1 dimension");

    const int64_t N = coeffs.size(-1);
    const int64_t B = coeffs.numel() / N;
    const int64_t output_N = N + 1;

    auto coeffs_flat = coeffs.reshape({B, N}).contiguous();
    auto output = at::empty({B, output_N}, coeffs.options());

    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "chebyshev_polynomial_w_antiderivative",
        [&] {
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            scalar_t* out_ptr = output.data_ptr<scalar_t>();

            at::parallel_for(0, B, 1, [&](int64_t start, int64_t end) {
                for (int64_t batch = start; batch < end; ++batch) {
                    kernel::polynomial::chebyshev_polynomial_w_antiderivative(
                        out_ptr + batch * output_N,
                        coeffs_ptr + batch * N,
                        N
                    );
                }
            });
        }
    );

    return output;
}

// Backward: grad_output (B, N+1), coeffs (B, N) -> grad_coeffs (B, N)
inline at::Tensor chebyshev_polynomial_w_antiderivative_backward(
    const at::Tensor& grad_output,
    const at::Tensor& coeffs
) {
    TORCH_CHECK(coeffs.dim() >= 1, "coeffs must have at least 1 dimension");

    const int64_t N = coeffs.size(-1);
    const int64_t B = coeffs.numel() / N;
    const int64_t output_N = N + 1;

    auto grad_output_flat = grad_output.reshape({B, output_N}).contiguous();
    auto coeffs_flat = coeffs.reshape({B, N}).contiguous();
    auto grad_coeffs = at::zeros({B, N}, coeffs.options());

    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "chebyshev_polynomial_w_antiderivative_backward",
        [&] {
            const scalar_t* grad_out_ptr = grad_output_flat.data_ptr<scalar_t>();
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            scalar_t* grad_coeffs_ptr = grad_coeffs.data_ptr<scalar_t>();

            at::parallel_for(0, B, 1, [&](int64_t start, int64_t end) {
                for (int64_t batch = start; batch < end; ++batch) {
                    kernel::polynomial::chebyshev_polynomial_w_antiderivative_backward(
                        grad_coeffs_ptr + batch * N,
                        grad_out_ptr + batch * output_N,
                        coeffs_ptr + batch * N,
                        N
                    );
                }
            });
        }
    );

    return grad_coeffs;
}

// Backward backward: gg_coeffs (B, N), grad_output (B, N+1), coeffs (B, N) -> grad_grad_output (B, N+1)
inline at::Tensor chebyshev_polynomial_w_antiderivative_backward_backward(
    const at::Tensor& gg_coeffs,
    const at::Tensor& grad_output,
    const at::Tensor& coeffs
) {
    TORCH_CHECK(coeffs.dim() >= 1, "coeffs must have at least 1 dimension");

    const int64_t N = coeffs.size(-1);
    const int64_t B = coeffs.numel() / N;
    const int64_t output_N = N + 1;

    auto gg_coeffs_flat = gg_coeffs.defined() ? gg_coeffs.reshape({B, N}).contiguous() : at::zeros({B, N}, coeffs.options());
    auto grad_output_flat = grad_output.reshape({B, output_N}).contiguous();
    auto coeffs_flat = coeffs.reshape({B, N}).contiguous();
    auto grad_grad_output = at::zeros({B, output_N}, grad_output.options());

    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "chebyshev_polynomial_w_antiderivative_backward_backward",
        [&] {
            const scalar_t* gg_coeffs_ptr = gg_coeffs_flat.data_ptr<scalar_t>();
            const scalar_t* grad_out_ptr = grad_output_flat.data_ptr<scalar_t>();
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            scalar_t* ggo_ptr = grad_grad_output.data_ptr<scalar_t>();

            at::parallel_for(0, B, 1, [&](int64_t start, int64_t end) {
                for (int64_t batch = start; batch < end; ++batch) {
                    kernel::polynomial::chebyshev_polynomial_w_antiderivative_backward_backward(
                        ggo_ptr + batch * output_N,
                        gg_coeffs_ptr + batch * N,
                        grad_out_ptr + batch * output_N,
                        coeffs_ptr + batch * N,
                        N
                    );
                }
            });
        }
    );

    return grad_grad_output;
}

} // namespace torchscience::cpu::polynomial

TORCH_LIBRARY_IMPL(torchscience, CPU, module) {
    module.impl("chebyshev_polynomial_w_antiderivative", torchscience::cpu::polynomial::chebyshev_polynomial_w_antiderivative);
    module.impl("chebyshev_polynomial_w_antiderivative_backward", torchscience::cpu::polynomial::chebyshev_polynomial_w_antiderivative_backward);
    module.impl("chebyshev_polynomial_w_antiderivative_backward_backward", torchscience::cpu::polynomial::chebyshev_polynomial_w_antiderivative_backward_backward);
}
