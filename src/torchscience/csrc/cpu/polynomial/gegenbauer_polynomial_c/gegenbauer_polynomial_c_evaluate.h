...
#include "../../../kernel/polynomial/gegenbauer_polynomial_c/gegenbauer_polynomial_c_evaluate.h"
#include "../../../kernel/polynomial/gegenbauer_polynomial_c/gegenbauer_polynomial_c_evaluate_backward.h"
#include "../../../kernel/polynomial/gegenbauer_polynomial_c/gegenbauer_polynomial_c_evaluate_backward_backward.h"
...
inline at::Tensor gegenbauer_polynomial_c_evaluate(
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        ...
        [&] {
            ...
            scalar_t alpha_val = alpha_contig.item<scalar_t>();
            ...
            at::parallel_for(0, B * M, 1, [&](int64_t start, int64_t end) {
                for (int64_t idx = start; idx < end; ++idx) {
                    int64_t b = idx / M;
                    int64_t m = idx % M;
                    out_ptr[idx] = kernel::polynomial::gegenbauer_polynomial_c_evaluate(
                        coeffs_ptr + b * N,
                        x_ptr[m],
                        alpha_val,
                        N
...
inline std::tuple<at::Tensor, at::Tensor, at::Tensor> gegenbauer_polynomial_c_evaluate_backward(
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "gegenbauer_polynomial_c_evaluate_backward",
        [&] {
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            const scalar_t* x_ptr = x_contig.data_ptr<scalar_t>();
            scalar_t alpha_val = alpha_contig.item<scalar_t>();
            ...
            scalar_t total_grad_alpha = scalar_t(0);

            for (int64_t b = 0; b < B; ++b) {
                for (int64_t m = 0; m < M; ++m) {
                    std::vector<scalar_t> temp_grad_coeffs(N);

                    auto [local_grad_x, local_grad_alpha] =
                        kernel::polynomial::gegenbauer_polynomial_c_evaluate_backward(
                    ...
                    total_grad_alpha += local_grad_alpha;
                }
            }

            grad_alpha.fill_(total_grad_alpha);
        }
    );
...
inline std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> gegenbauer_polynomial_c_evaluate_backward_backward(
    ...
    auto gg_alpha_contig = gg_alpha.defined() ? gg_alpha.contiguous() : at::zeros({}, alpha.options());
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::kBFloat16, at::kHalf,
        coeffs.scalar_type(),
        "gegenbauer_polynomial_c_evaluate_backward_backward",
        [&] {
            const scalar_t* coeffs_ptr = coeffs_flat.data_ptr<scalar_t>();
            const scalar_t* x_ptr = x_contig.data_ptr<scalar_t>();
            scalar_t alpha_val = alpha_contig.item<scalar_t>();
            ...
            scalar_t gg_alpha_val = gg_alpha_contig.item<scalar_t>();
            ...
            for (int64_t b = 0; b < B; ++b) {
                for (int64_t m = 0; m < M; ++m) {
                    ...
                    auto [ggo, g_x, g_alpha] =
                        kernel::polynomial::gegenbauer_polynomial_c_evaluate_backward_backward(
                            temp_grad_coeffs.data(),
                            gg_coeffs_ptr + b * N,
                            gg_x_ptr[m],
                            gg_alpha_val,
    ...
    return {grad_grad_output, grad_coeffs, grad_x, grad_alpha};
}

} // namespace torchscience::cpu::polynomial

TORCH_LIBRARY_IMPL(torchscience, CPU, module) {
    module.impl("gegenbauer_polynomial_c_evaluate", torchscience::cpu::polynomial::gegenbauer_polynomial_c_evaluate);
    module.impl("gegenbauer_polynomial_c_evaluate_backward", torchscience::cpu::polynomial::gegenbauer_polynomial_c_evaluate_backward);
    module.impl("gegenbauer_polynomial_c_evaluate_backward_backward", torchscience::cpu::polynomial::gegenbauer_polynomial_c_evaluate_backward_backward);
}
