...
#include "../../kernel/morphology/dilation.h"
#include "../../kernel/morphology/dilation_backward.h"

namespace torchscience::cpu::morphology {

/**
 * CPU implementation for N-dimensional dilation.
 *
 ...
 * @return Dilated tensor with same shape as input
 */
inline at::Tensor dilation(
    ...
    if (origin.has_value()) {
        TORCH_CHECK(origin->size() == static_cast<size_t>(ndim),
            "dilation: origin must have same number of dimensions as structuring_element");
    }
        ...
        [&] {
            ...
            at::parallel_for(0, num_batch * num_spatial, 0, [&](int64_t begin, int64_t end) {
                for (int64_t idx = begin; idx < end; ++idx) {
                    ...
                    batch_output[spatial_idx] = kernel::morphology::dilation_scalar(
                        batch_input,
                        se_ptr,
                        se_mask_ptr,
                        spatial_idx,
...
/**
 * CPU implementation for dilation backward pass.
 */
        ...
        [&] {
            ...
            for (int64_t batch_idx = 0; batch_idx < num_batch; ++batch_idx) {
                ...
                for (int64_t spatial_idx = 0; spatial_idx < num_spatial; ++spatial_idx) {
                    kernel::morphology::dilation_backward_scalar(
...
TORCH_LIBRARY_IMPL(torchscience, CPU, m) {
    m.impl("dilation", &torchscience::cpu::morphology::dilation);
    m.impl("dilation_backward", &torchscience::cpu::morphology::dilation_backward);
}
