...
/**
 * Meta implementation of inverse Fourier transform for shape inference.
 *
 * The inverse Fourier transform preserves the shape of the input tensor,
 * unless n is specified, in which case the output size along dim is n.
 * Output is complex (unless real input and output is taken).
 */
...
TORCH_LIBRARY_IMPL(torchscience, Meta, module) {
    module.impl(
        ...
        &torchscience::meta::transform::inverse_fourier_transform
    );

    module.impl(
        "inverse_fourier_transform_backward",
        &torchscience::meta::transform::inverse_fourier_transform_backward
    );

    module.impl(
        "inverse_fourier_transform_backward_backward",
        &torchscience::meta::transform::inverse_fourier_transform_backward_backward
    );
}
