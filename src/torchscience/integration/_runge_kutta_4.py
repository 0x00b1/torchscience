...
from torchscience.integration._interpolation import (
    HermiteInterpolant,
    LinearInterpolant,
)
...
def runge_kutta_4(
    ...
    dt: float,
    throw: bool = True,
    dense_output: Literal["linear", "hermite"] = "linear",
) -> Tuple[
    ...
    dense_output : {"linear", "hermite"}
        Type of interpolation for dense output. "linear" (default) uses piecewise
        linear interpolation (O(dt^2) accuracy). "hermite" uses cubic Hermite
        interpolation with derivatives (O(dt^4) accuracy) - significantly more
        accurate at midpoints between grid nodes.
    ...
    # Compute initial derivative for Hermite interpolation
    use_hermite = dense_output == "hermite"
    if use_hermite:
        dy_points = [f_flat(t, y).clone()]
    ...
    while direction * (t1 - t) > 1e-10:
        ...
        # Store derivative at new point for Hermite interpolation
        if use_hermite:
            # k4 is f(t_{n+1}, y_n + h*k3), but we need f(t_{n+1}, y_{n+1})
            # So we need to compute it properly at the new state
            dy_points.append(f_flat(t, y).clone())
    ...
    if use_hermite:
        interp = HermiteInterpolant(
            t_tensor, y_tensor, dy_tensor, success=success
        )
    else:
