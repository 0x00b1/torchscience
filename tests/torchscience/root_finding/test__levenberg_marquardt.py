...
class TestLevenbergMarquardtCompile:
    ...
    def test_compile_eager_backend(self):
        """torch.compile with eager backend works with levenberg_marquardt."""
    ...
    def test_compile_inductor_backend(self):
        """torch.compile with inductor backend works with levenberg_marquardt."""
    ...
    def test_compile_damping_parameter(self):
        """torch.compile works with different damping parameters."""

        def solve(x0, damping):
            def f(x):
                x1, x2 = x[..., 0], x[..., 1]
                f1 = x1**2 + x2**2 - 1
                f2 = x1 - x2
            ...
            root, converged = levenberg_marquardt(f, x0, damping=damping)
            return root

        solve_compiled = torch.compile(solve, backend="eager")

        x0 = torch.tensor([0.5, 0.5], dtype=torch.float64)
        root = solve_compiled(x0, 0.1)
