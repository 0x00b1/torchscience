...
class TestTrustRegionCompile:
    ...
    def test_compile_eager_backend(self):
        """torch.compile with eager backend works with trust_region."""
    ...
    def test_compile_inductor_backend(self):
        """torch.compile with inductor backend works with trust_region."""
    ...
    def test_compile_radius_parameters(self):
        """torch.compile works with custom radius parameters."""

        def solve(x0, initial_radius, max_radius):
            def f(x):
                x1, x2 = x[..., 0], x[..., 1]
                f1 = x1**2 + x2**2 - 1
                f2 = x1 - x2
            ...
            root, converged = trust_region(
                f, x0, initial_radius=initial_radius, max_radius=max_radius
            )
            return root
        ...
        x0 = torch.tensor([0.5, 0.5], dtype=torch.float64)
        root = solve_compiled(x0, 0.5, 10.0)
