...
class TestNewtonVmap:
    """Tests for vmap compatibility.

    Note: vmap is currently NOT compatible with newton due to:
    ...
    (e.g., newton(f, batched_x0)) instead of vmap for vectorized computation.
    """
    ...
    def test_vmap_basic(self):
        """vmap works with newton for vectorized parameter sweeps."""
    ...
    def test_vmap_explicit_derivative(self):
        """vmap works with explicit derivative function."""
        from torch.func import vmap
    ...
    def test_explicit_batching_alternative(self):
        ...
        Instead of using vmap, pass batched inputs directly to newton.
        """
        c = torch.tensor([2.0, 3.0, 4.0], dtype=torch.float64)
        f = lambda x: x**2 - c
        x0 = torch.full((3,), 1.5, dtype=torch.float64)
...
class TestNewtonCompile:
    ...
    def test_compile_eager_backend(self):
        """torch.compile with eager backend works with newton."""
    ...
    def test_compile_inductor_backend(self):
        """torch.compile with inductor backend works with newton."""
        c = torch.tensor([2.0, 3.0, 4.0])
    ...
    def test_compile_explicit_derivative(self):
        """torch.compile works with explicit derivative function."""
        c = torch.tensor([2.0, 3.0, 4.0])
