...
class TestInverseLaplaceTransformVmap:
    ...
    def test_vmap_basic(self):
        ...
        # Manual batching
        f_batched = T.inverse_laplace_transform(F, t, s, sigma=sigma, dim=-1)

        # vmap
        def inv_laplace_single(Fi):
            return T.inverse_laplace_transform(Fi, t, s, sigma=sigma)
...
class TestInverseLaplaceTransformCompile:
    """Test inverse Laplace transform with torch.compile."""
    ...
    def test_compile_basic(self):
        ...
        def compiled_inv_laplace(x):
            return T.inverse_laplace_transform(x, t, s, sigma=sigma)

        f_compiled = compiled_inv_laplace(F)
        f_eager = T.inverse_laplace_transform(F, t, s, sigma=sigma)
