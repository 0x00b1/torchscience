"""Tests for Störmer-Verlet (velocity Verlet) symplectic integrator."""

import math

import torch

from torchscience.integration._stormer_verlet import (
    stormer_verlet,
)


class TestStormerVerletBasic:
    def test_harmonic_oscillator(self):
        """Simple harmonic oscillator: H = p^2/2 + q^2/2."""

        def grad_potential(t, q):
            return q  # dV/dq = q for V = q^2/2

        def grad_kinetic(t, p):
            return p  # dT/dp = p for T = p^2/2

        q0 = torch.tensor([1.0], dtype=torch.float64)
        p0 = torch.tensor([0.0], dtype=torch.float64)

        q_final, p_final, interp = stormer_verlet(
            grad_potential,
            grad_kinetic,
            q0,
            p0,
    ...
    def test_energy_conservation(self):
        """Energy should be conserved (bounded, not drifting) over long integration."""
        ...
        # Integrate for 100 periods
        q_final, p_final, interp = stormer_verlet(
            grad_potential,
            grad_kinetic,
            q0,
            p0,
            t_span=(0.0, 200 * math.pi),
            dt=0.01,
        )

        E_final = energy(q_final, p_final).item()

        # Energy should be conserved to within O(h^2) per period, bounded overall
        assert abs(E_final - E0) < 0.1  # Energy bounded, not drifting

    def test_kepler_problem(self):
        """Two-body Kepler problem: circular orbit should remain circular."""

        def grad_potential(t, q):
            # V = -1/|q|, so grad V = q/|q|^3
            r = torch.norm(q)
            return q / (r**3 + 1e-10)
        ...
        # Circular orbit with r=1, v=1
        q0 = torch.tensor([1.0, 0.0], dtype=torch.float64)
        p0 = torch.tensor([0.0, 1.0], dtype=torch.float64)

        # One orbit period is 2*pi for these initial conditions
        q_final, p_final, _ = stormer_verlet(
            grad_potential,
            grad_kinetic,
            q0,
            p0,
        ...
        # Should return close to initial (circular orbit is periodic)
...
class TestStormerVerletInterpolant:
...
class TestStormerVerletAutograd:
    def test_gradient_through_solver(self):
        """Gradients should flow through symplectic solver."""
...
class TestStormerVerletMultidimensional:
    ...
    def test_anisotropic_oscillator(self):
        ...
        assert abs(E_final - E0) < 0.1


class TestStormerVerletSpecialCases:
    ...
    def test_large_step_size(self):
        ...
        assert torch.allclose(q_final, q0, atol=0.2)
        assert torch.allclose(p_final, p0, atol=0.2)
