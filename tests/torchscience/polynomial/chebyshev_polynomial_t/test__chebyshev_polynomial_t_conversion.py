...
class TestChebyshevPolynomialTToPolynomial:
    ...
    def test_linear_combination(self):
        ...
        p = chebyshev_polynomial_t_to_polynomial(c)
        torch.testing.assert_close(p, torch.tensor([-2.0, 2.0, 6.0]))
    ...
    def test_vs_numpy(self):
        ...
        np.testing.assert_allclose(p.numpy(), p_np, rtol=1e-10)
...
class TestPolynomialToChebyshevPolynomialT:
    ...
    def test_x3_to_cheb(self):
        ...
        c = polynomial_to_chebyshev_polynomial_t(p)
        torch.testing.assert_close(c, torch.tensor([0.0, 0.75, 0.0, 0.25]))
    ...
    def test_roundtrip_cheb_power_cheb(self):
        ...
        torch.testing.assert_close(c_back, coeffs_orig, atol=1e-10, rtol=1e-10)
    ...
    def test_roundtrip_power_cheb_power(self):
        ...
        torch.testing.assert_close(p_back, coeffs_orig, atol=1e-10, rtol=1e-10)
    ...
    def test_vs_numpy(self):
        ...
        np.testing.assert_allclose(c.numpy(), c_np, rtol=1e-10)
...
class TestConversionAutograd:
    ...
    def test_to_polynomial_gradcheck(self):
        ...
        def fn(c):
            result = chebyshev_polynomial_t_to_polynomial(
    ...
    def test_from_polynomial_gradcheck(self):
        ...
        def fn(c):
            result = polynomial_to_chebyshev_polynomial_t(polynomial(c))
            return result.as_subclass(torch.Tensor)
