...
class TestRampWave:
    def test_basic_shape(self):
        result = torchscience.waveform.ramp_wave(n=100)
        assert result.shape == (100,)

    def test_ramp_at_origin(self):
        result = torchscience.waveform.ramp_wave(n=100, position=0, slope=1.0)
        expected = torch.arange(100, dtype=result.dtype)
        torch.testing.assert_close(result, expected)

    def test_ramp_at_position(self):
        result = torchscience.waveform.ramp_wave(n=100, position=50, slope=2.0)
    ...
    def test_batched_positions(self):
        ...
        result = torchscience.waveform.ramp_wave(n=100, position=positions)
