"""Tests for lchab_to_srgb color conversion."""
...
from torchscience.color import lchab_to_srgb


class TestLchabToSrgbShape:
    """Tests for shape handling."""

    def test_single_pixel(self):
        """Single pixel (3,) input."""
        lch = torch.tensor([50.0, 25.0, 0.5])
        rgb = lchab_to_srgb(lch)
        assert rgb.shape == (3,)

    def test_batch_1d(self):
        """Batch of pixels (B, 3)."""
        lch = torch.zeros(10, 3)
        lch[:, 0] = torch.rand(10) * 100  # L in [0, 100]
        lch[:, 1] = torch.rand(10) * 50  # C in [0, 50]
    ...
    def test_invalid_last_dim(self):
        ...
        with pytest.raises(ValueError, match="last dimension 3"):
            lchab_to_srgb(torch.randn(10, 4))


class TestLchabToSrgbKnownValues:
    """Tests for known color conversions."""

    def test_white(self):
        """White: L=100, C=0 -> (1, 1, 1)."""
    ...
    def test_red_approximate(self):
        """Approximate red: known Lab values converted to LCH."""
        # Red in Lab: L~53.23, a~80.11, b~67.22
        # LCH: L~53.23, C~sqrt(80.11^2+67.22^2)~104.55, h~atan2(67.22,80.11)~0.70
        L = 53.2329
        a = 80.1093
        b = 67.2201
        C = math.sqrt(a**2 + b**2)
    ...
    def test_mid_gray(self):
        """Mid gray: L=50, C=0 -> gray RGB."""
        lch = torch.tensor([53.389, 0.0, 0.0])  # L* for sRGB (0.5, 0.5, 0.5)
        rgb = lchab_to_srgb(lch)
    ...
    def test_hue_invariance_at_zero_chroma(self):
        ...
        L = 50.0
        C = 0.0
        rgb_h0 = lchab_to_srgb(torch.tensor([L, C, 0.0]))
        rgb_h1 = lchab_to_srgb(torch.tensor([L, C, math.pi / 2]))
        rgb_h2 = lchab_to_srgb(torch.tensor([L, C, math.pi]))
        rgb_h3 = lchab_to_srgb(torch.tensor([L, C, -math.pi / 2]))
...
class TestLchabToSrgbGradients:
    ...
    def test_gradcheck(self):
        ...
        lch_data = lch.data
        lch_data[:, 0] = (
            torch.rand(5, dtype=torch.float64) * 80 + 10
        )  # L in [10, 90]
        lch_data[:, 1] = (
            torch.rand(5, dtype=torch.float64) * 40 + 10
        )  # C in [10, 50]
        lch_data[:, 2] = (
            torch.rand(5, dtype=torch.float64) * 2 * math.pi - math.pi
        )
        lch = lch_data.clone().requires_grad_(True)
        assert gradcheck(lchab_to_srgb, (lch,), eps=1e-6, atol=1e-4)

    def test_gradcheck_batch(self):
        """Gradient check on batch."""
        lch = torch.zeros(4, 4, 3, dtype=torch.float64)
        lch[:, :, 0] = torch.rand(4, 4, dtype=torch.float64) * 80 + 10
        lch[:, :, 1] = torch.rand(4, 4, dtype=torch.float64) * 40 + 10
        lch[:, :, 2] = (
    ...
    def test_gradient_at_zero_chroma(self):
        ...
        lch = torch.tensor(
            [[50.0, 0.0, 0.5]], dtype=torch.float64, requires_grad=True
        )
        rgb = lchab_to_srgb(lch)
        loss = rgb.sum()
        loss.backward()
...
class TestLchabToSrgbDtypes:
    """Tests for different data types."""

    def test_float32(self):
        """Test with float32."""
        lch = torch.tensor([[50.0, 25.0, 0.5]], dtype=torch.float32)
        rgb = lchab_to_srgb(lch)
        assert rgb.dtype == torch.float32

    def test_float64(self):
        """Test with float64."""
        lch = torch.tensor([[50.0, 25.0, 0.5]], dtype=torch.float64)
        rgb = lchab_to_srgb(lch)
        assert rgb.dtype == torch.float64

    def test_bfloat16(self):
        """Test with bfloat16."""
        lch = torch.tensor([[50.0, 25.0, 0.5]], dtype=torch.bfloat16)
        rgb = lchab_to_srgb(lch)
        assert rgb.dtype == torch.bfloat16

    def test_float16(self):
        """Test with float16."""
        lch = torch.tensor([[50.0, 25.0, 0.5]], dtype=torch.float16)
        rgb = lchab_to_srgb(lch)
        assert rgb.dtype == torch.float16


class TestLchabToSrgbEdgeCases:
    """Tests for edge cases."""

    def test_high_lightness(self):
        """Test with L > 100 (HDR-like)."""
        lch = torch.tensor([120.0, 10.0, 0.0])
        rgb = lchab_to_srgb(lch)
    ...
    def test_negative_lightness(self):
        ...
        lch = torch.tensor([-10.0, 10.0, 0.0])
        rgb = lchab_to_srgb(lch)
        assert not torch.any(torch.isnan(rgb))
    ...
    def test_large_chroma(self):
        ...
        lch = torch.tensor([50.0, 150.0, 0.0])  # C=150 is very saturated
        rgb = lchab_to_srgb(lch)
        assert not torch.any(torch.isnan(rgb))
        # Result will be out of gamut (some values < 0 or > 1)


class TestLchabToSrgbConsistency:
    """Tests for consistency with Lab conversion."""

    def test_consistent_with_lab(self):
        """LCHab -> sRGB should match Lab -> sRGB when converted properly."""
        from torchscience.color import lab_to_srgb

        # Create some LCH values
        L = torch.rand(10) * 80 + 10  # L in [10, 90]
        C = torch.rand(10) * 50 + 10  # C in [10, 60]
        h = torch.rand(10) * 2 * math.pi - math.pi

        lch = torch.stack([L, C, h], dim=-1)

        # Convert to Lab manually
        a = C * torch.cos(h)
        b = C * torch.sin(h)
        lab = torch.stack([L, a, b], dim=-1)

        # Compare outputs
        rgb_from_lch = lchab_to_srgb(lch)
        rgb_from_lab = lab_to_srgb(lab)

        assert torch.allclose(rgb_from_lch, rgb_from_lab, atol=1e-5)


class TestLchabToSrgbRoundTrip:
    """Round-trip conversion tests."""

    def test_roundtrip(self):
        """LCHab -> sRGB -> LCHab should recover original."""
        from torchscience.color import srgb_to_lchab

        # Start with valid LCH values that produce in-gamut RGB
        L = torch.rand(100) * 80 + 10  # L in [10, 90]
        C = torch.rand(100) * 30 + 5  # C in [5, 35] (conservative)
        ...
        lch_recovered = srgb_to_lchab(rgb)
        rgb_recovered = lchab_to_srgb(lch_recovered)
    ...
    def test_roundtrip_gradients(self):
        ...
        L = torch.rand(10) * 60 + 20
        C = torch.rand(10) * 20 + 5
        h = torch.rand(10) * 2 * math.pi - math.pi
    ...
    def test_srgb_roundtrip(self):
        """sRGB -> LCHab -> sRGB should recover original."""
        from torchscience.color import srgb_to_lchab

        rgb = torch.rand(100, 3) * 0.9 + 0.05  # Avoid extremes
        lch = srgb_to_lchab(rgb)
        rgb_recovered = lchab_to_srgb(lch)
