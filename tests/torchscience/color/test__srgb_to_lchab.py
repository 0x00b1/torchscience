"""Tests for srgb_to_lchab color conversion."""
...
class TestSrgbToLchabShape:
    ...
    def test_invalid_last_dim(self):
        ...
        with pytest.raises(ValueError, match="last dimension 3"):
            srgb_to_lchab(torch.randn(10, 4))


class TestSrgbToLchabKnownValues:
    ...
    def test_red(self):
        ...
        lch = srgb_to_lchab(rgb)
        # Reference: L* ~ 53.23, C* ~ 104.55, h ~ 0.69 rad (40 degrees)
        assert torch.isclose(lch[0], torch.tensor(53.2329), atol=0.1)
        # C = sqrt(a^2 + b^2) where a~80.1, b~67.2
        expected_c = math.sqrt(80.1093**2 + 67.2201**2)
        assert torch.isclose(lch[1], torch.tensor(expected_c), atol=1.0)
        # h = atan2(b, a)
        expected_h = math.atan2(67.2201, 80.1093)
    ...
    def test_green(self):
        ...
        assert lch[1] > 0
        # h should be in the green region (around 2.23 rad or 128 degrees)
        # a ~ -86.18, b ~ 83.18, so h = atan2(83.18, -86.18) ~ 2.38 rad
        expected_h = math.atan2(83.1812, -86.1846)
        assert torch.isclose(lch[2], torch.tensor(expected_h), atol=0.02)
    ...
    def test_blue(self):
        ...
        # a ~ 79.20, b ~ -107.86, so h should be negative (around -0.94 rad)
        expected_h = math.atan2(-107.8637, 79.1967)
        assert torch.isclose(lch[2], torch.tensor(expected_h), atol=0.02)
...
class TestSrgbToLchabGradients:
    ...
    def test_gradcheck(self):
        ...
        assert gradcheck(srgb_to_lchab, (rgb,), eps=1e-6, atol=1e-4)
...
class TestSrgbToLchabEdgeCases:
...
class TestSrgbToLchabConsistency:
    """Tests for consistency with Lab conversion."""

    def test_consistent_with_lab(self):
        """LCH should be consistent with Lab (L and C=sqrt(a^2+b^2))."""
...
class TestSrgbToLchabRoundTrip:
